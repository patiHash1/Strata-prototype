...
**Files:** `super_admin.go` · `super_admin_types.go` · `super_admin_repo.go` · `super_admin_telemetry.go` · `super_admin_maintenance.go` · `super_admin_soc.go` · `super_admin_health.go`

System observability is split into four focused modules behind a composition-root container:

- **`Telemetry`** — runtime/DB/HTTP metrics, latency + traffic ring buffers, `CollectSnapshot`, `TrafficSeries`, `PrometheusMetrics`, `PerModuleMetrics`. Middleware pushes records in via `RecordHTTPLatency` / `RecordPanic`.
- **`SOCMonitor`** — security-event ingestion, recent-event buffer, SSE subscriber fan-out, Redis pub/sub + retention pruning.
- **`Maintenance`** — partitioned-maintenance rule cache, `IsUnderMaintenance`, `ToggleMaintenance`, `DeleteMaintenanceRule`, `List*`, Redis cache-invalidation sync.
- **`ModuleHealthSvc`** — `GetModuleHealth`, `GetModuleStatus`, `IngestCIHealth`; composes `Telemetry` + `Maintenance` through their interfaces (`HTTPMetricsSource`, `MaintenanceStatusSource`).

`SuperAdmin` is the composition root: it owns the shared `pool`/`redis` lifecycle, constructs the four modules, exposes `PingRedis()`, and delegates `Shutdown()` to each module. It is not a facade — it re-exports no domain methods.

**Dependencies:** `*pgxpool.Pool`, `*redis.Client` (optional), `ActiveUserCounter` (for `Telemetry`)

**Redis channels:**
- `strata:events:maintenance-sync` — cache invalidation broadcast
- `strata:events:security-soc` — SOC event fan-out for SSE

**Ring buffers:** Fixed-size buffers for panics, latencies, telemetry snapshots, and traffic buckets — prevents unbounded memory growth under load.
